...
exports.Factory.prototype.parseSitemap = function (jsonSitemap) {
    ...
    for (var key in this.itemList) {
        if (this.itemList.hasOwnProperty(key)){
            ...
            if (this.platform.outputs[o] !== "") {
                for (var i = 0; i < this.platform.inputs.length; i++) {
                    if (this.platform.inputs[i] !== "") {
                        this.itemList[key].name = this.platform.inputs[i] + " " + this.platform.outputs[o];
                        this.log("Platform - Accessory Found: " + this.itemList[key].name);
                        accessoryList.push(new exports.SwitchItem(this.itemList[key], o, i, this.platform, this.homebridge));
                        devicesProcessed++;
                    }
                }
            }
            this.log("Platform - Accessory Found: " + this.itemList[key].label);
            this.log("Platform - Devices Processed: " + devicesProcessed);
